...
if __name__ == '__main__':
    ...
    base_root = '/mnt/edward/data/adityaj/nnUNet_2022/Step_0_data_formatting/N_CALC_folder_3D_New/'
    ...
    for idx in range(train_startidx, train_stopidx):
        ...
        if flag_crop:
            input_image_file = mat_contents['I'][tb:bb,lb:rb,:]
        else:
            input_image_file = mat_contents['I']
        mat_contents_seg = sio.loadmat(base + 'Signal_'+str(idx)+'/masks/mask.mat')
        if flag_crop:
            input_segmentation_file = mat_contents_seg['Mask'][tb:bb,lb:rb,:]
        else:
            input_segmentation_file = mat_contents_seg['Mask']
        ...
        load_tiff_convert_to_nifti(input_image_file, input_segmentation_file, output_image_file, output_seg_file, spacing)
        
        #load_tiff_convert_to_nifti(input_image_file, output_image_file, is_seg=False)
        ...
        #load_tiff_convert_to_nifti(input_segmentation_file, output_seg_file, is_seg=True,
    ...
    > nnUNet_plan_and_preprocess -t 703 --verify_dataset_integrity
 
    once that is completed, you can run the trainings as follows:
    > nnUNet_train 3d_fullres nnUNetTrainerV2 703 0   
