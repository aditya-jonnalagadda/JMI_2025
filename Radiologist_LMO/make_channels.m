...
function [U] = make_channels(chanType,Nx,Ny,dx, flag_calc)
...
switch chanType
  case 1,  % Gabor functions  
     if flag_calc
        pb = [1/64,1/32,1/16,1/8,1/4,1/2];%,1/1];%,1/0.5]; %
     else
        pb = [1/64,1/32,1/16,1/8,1/4,1/2];  % edges of passbands (cycles/pixel)
     end
     wf = diff(pb); % bandwidths
     ws = (length(wf)/pi*log(2)./wf)*dx;  % spatial channel widths (in cm)
     fc = (pb(1:(length(pb)-1))+wf/2)/dx;    % center frequencies (in cycles/cm)
     num_angles = 8;
     dtheta = 1/num_angles;
     theta = (0:dtheta:(1-dtheta))*pi;  % orientations
     C = zeros(Nx,Ny);
     U = zeros(Nx*Ny,2*(length(pb)-1)*num_angles);   % channel matrix
     x2 = x - X(1);
     y2 = y - X(2);
     beta = [0;pi/2];
     count = 0;
     for p=1:2, % loop over phases
        for l=1:(length(pb)-1), % loop over frequencies
           for k=1:num_angles,  % loop over angles
              count = count +1;
              for i=1:Nx, % x-loop
                 for j=1:Ny, % y-loop
